...
impl AppState1RawData{
    pub fn save_to_partial_zip(&self, sink: impl std::io::Write + std::io::Seek){
        let config: Option<JsonValue> = match &self.custom_config_widget {
            None => None,
            Some(state) => serde_json::from_str(&state.code_editor_widget.raw).ok()
        };
        let mut out = serde_json::json!({
            "name": self.staging_name,
            "description": self.staging_description,
            // pub cover_images: Vec<SpecialImageWidgetRawData>,
            "model_id_widget": self.model_id_widget,
            "authors": self.staging_authors,
            // pub attachments_widget: Vec<FileSourceWidgetRawData>,
            "citationsstaging_citations": self.staging_citations,
            // #[serde(default)] // added after AppState1RawData
            "config": config,
            "git_repo": self.staging_git_repo,
            // pub icon_widget: Option<IconWidgetRawData>,
            // #[serde(default)] // added after AppState1RawData
            "links": self.links_widget,
            "maintainers": self.staging_maintainers,
            "tags": self.staging_tags,
            "version": self.staging_version,

            // pub staging_documentation: CodeEditorWidgetRawData,
            "license": self.staging_license,
            // //badges
            // pub model_interface_widget: ModelInterfaceWidgetRawData,
            // ////
            // pub weights_widget: WeightsWidgetRawData,
